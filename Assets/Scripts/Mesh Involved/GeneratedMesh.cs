...
public class GeneratedMesh : MonoBehaviour
{
	...
	private void Generate()
	{
		...
		for (int i = 0, z = 0; z <= zSize-1; z++)
		{
			for (int y = 0; y <= ySize-1; y++)
			{
					for (int x = 0; x <= xSize-1; x++, i++)
					{
						...
						uv[i] = new Vector3((float)x / xSize, (float)y / ySize, (float)z / zSize);
						tangents[i] = tangent;
					}
			}
		}
		...
		int[] triangles = new int[(int)(xSize * ySize * zSize * 6)];
		for (int ti = 0, vi = 0, z = 0; z < zSize-1; z++, vi++)
		{
			for (int y1 = 0; y1 < ySize1-1; y1++)
			{
